...
const { UserModel, FollowModel, PublicationModel } = require("../Models/Index.models");
const { generateToken } = require("../Services/JWT.service");
const { ERROR, SUCCESS } = require("../enum/status_response.enum");
const {ExcludeFieldsHelper} = require('../Helpers/Index.helper');
const fs = require("fs");
...
const UserController = {
    ...
    getProfileById: async (req, res) => {
        try {
            ...
            const user = await UserModel.findById(id)
                .select(ExcludeFieldsHelper.user.split(' '))
                .exec();
    ...
    updateUser: async (req, res) => {
        try {
            ...
            if (!validateObject(updateUser, ["email", "nick"]))
                return res.status(400).json({
                    status: ERROR,
            ...
            const users = await UserModel.find({
                $or: [{ email: updateUser.email }, { nick: updateUser.nick }],
            }).exec();

            const isUserDuplicated = users.some(
                (user) => user._id.toString() !== sesionUser.userId,
            );
    ...
    counters: async (req, res) =>{
        try {
            ...
            const user = await UserModel.findById(id, {name: 1, last_name: 1}).exec();
            ...
            const followedCount = await FollowModel.countDocuments({
                'followed_user': id
            }).exec();
            
            const publicationsCount = await PublicationModel.countDocuments({
                'user_id': id
            }).exec();
